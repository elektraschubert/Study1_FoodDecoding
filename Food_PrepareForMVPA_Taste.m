...
eventsOfInterest = {'TasteImage'}; % Implementation Stage: {'ImagePresentation'}, Anticipation Stage: {'DistractSymbol'} or {'ReappraiseSymbol'}
...
for subject = 1:length(subjectsToAnalyse)
    ...
    % Extract only specified trials (Taste trials)
    clear taste_trial;
    clear taste_trial_epoched_data;
    
    for eventNo = 1:numel(EEG.event)
        
        if strcmp(EEG.event(eventNo).type, 'TasteImage')
            
            taste_trial(EEG.event(eventNo).epoch) = 1;
            
        end % of if strcmp
    end % of for eventNo
    
    % Extract only the specified EEG data epochs into a matrix
    taste_trial_epoched_data = EEG.data(:,:, taste_trial == 1);
    ...
    trial_numbers_by_epoch = trial_numbers_by_epoch_temp(taste_trial == 1);
             
    
    % Save data in eeg_sorted_cond cell arrays
     eeg_sorted_cond{1,1} = permute(taste_trial_epoched_data, [2, 1, 3]); 

    
    % Save the EEG data
    save(['DDTBOX Data/Taste/ID', int2str(subjectsToAnalyse(subject)), '_CSD_Taste'], 'eeg_sorted_cond'); 

    % Generate SVR labels
    tasteRatings = ({EEG.event(:).tasteRating}).';
    SVR_labels = cell2mat(tasteRatings);
    save(['DDTBOX Data/Taste/ID', int2str(subjectsToAnalyse(subject)), '_Taste_regress_sorted_data'], 'SVR_labels'); 
    clear SVR_labels
