...
while(i<=length(varargin))
    switch lower(varargin{i})
        ...
        case 'aborttime'
            ...
            i             = i + 1;
        case 'foodImageDisplay'
            i             = i + 1;
            foodImageDisplay  = varargin{i};
            i             = i + 1;
            imageSize     = size(foodImageDisplay);
            stimuli       = Screen('MakeTexture', screenPointer, foodImageDisplay);
            drawImage     = 1; 
...
while answer == 0
    ...
    if drawImage == 1
         Screen('DrawTexture', screenPointer, foodImageDisplay,[] ,[] , 0);
    end
    
    % Drawing the question as text
    DrawFormattedText(screenPointer, question, 'center', rect(4)*(scalaPosition + 0.1), [0 0 0]); 
...
%     Screen('DrawLine', screenPointer, scaleColor, midTick(1), midTick(2), midTick(3), midTick(4), width);         % Mid tick
    ...
    if rangeType == 1
        ...
        position = 100-(position);                        % Reverses the position (e.g. 20 becomes 80)
    elseif rangeType == 2
